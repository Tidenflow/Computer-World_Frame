...
const visibleNodes = computed(() => {
  ...
  return nodesWithPositions.value.filter(node => {
    ...
    if (node.visibility === 'Outlined') return true;
...
function computeFocusZoom(nodeId: string): number {
  ...
  const targetNode = mapStore.frameMap.projection.nodeById[nodeId];
  if (!targetNode) return 1;

  const adjacentIds = new Set<string>(targetNode.deps);
  for (const [candidateId, children] of Object.entries(mapStore.frameMap.projection.childrenById)) {
    if (children.includes(nodeId)) adjacentIds.add(candidateId);
  }
  ...
  const densityScore = unlockedNeighbors * 1.0 + outlinedNeighbors * 0.45 + visibleEdges * 0.25;
  return clampZoom(1.0 + getDensityAdjustment(densityScore) + getWeightAdjustment(Math.max(1, 7 - targetNode.stage)));
}
